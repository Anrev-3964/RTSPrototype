...
class RTSPROTOTYPE_API UUnitData : public UDataAsset
{
	...
	float AttackDelay;
	UPROPERTY(EditDefaultsOnly, Category = "Unit Stats",
		meta = (Tooltip = "la velocita massima che l'unita puo raggiungere"))
	...
	UPROPERTY(EditDefaultsOnly, Category = "Unit Settings",
		meta = (Tooltip = "l'animazione usata dal unita quando attaca"))
	UAnimMontage* AttackMontage;
	UPROPERTY(EditDefaultsOnly, Category = "Unit AI ",
		meta = (Tooltip = "Range di attaco per l'unita (AttackRange = 200 se : AttackRange < 200)"))
	float AttackRange;
	UPROPERTY(EditDefaultsOnly, Category = "Unit AI ",
		meta = (Tooltip =
			"Range tolleranza per il navmesh dell'unita (UnitAcceptanceRadius = 100 se : UnitAcceptanceRadius < 100)"))
